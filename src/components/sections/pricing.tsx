...
const Pricing = () => {
  ...
  return (
    <section id="precos" className="relative z-10 section-padding bg-slate-50/50">
      <div className="mx-auto max-w-5xl container-mobile">
        <div className="mb-8 sm:mb-12 text-center">
          ...
          <p className="mx-auto mt-2 max-w-md text-[13px] sm:text-[14px] text-slate-500 font-medium leading-relaxed">
            {t("subtitle")}
          </p>
        </div>
        ...
        <div className="mx-auto grid max-w-3xl gap-3 sm:gap-4 md:grid-cols-2">
          {plans.map((plan) => (
            <div
              key={plan.name}
              className={`relative rounded-[20px] p-4 sm:p-5 bg-white border transition-all duration-[0.4s] group ${
                plan.popular
                  ? "ring-2 ring-primary/20 border-primary/20 shadow-[0_4px_24px_-4px_rgba(111,0,255,0.1)] hover:shadow-[0_8px_32px_-4px_rgba(111,0,255,0.15)]"
                  : "border-slate-200/60 shadow-[0_2px_12px_-2px_rgba(0,0,0,0.04)] hover:shadow-[0_4px_20px_-4px_rgba(0,0,0,0.08)]"
              }`}
            >
              {plan.popular && (
                <div className="absolute -top-2 left-1/2 -translate-x-1/2 z-20">
                  <span className="px-2.5 py-1 text-[8px] sm:text-[9px] font-semibold uppercase tracking-wider text-white bg-primary rounded-full shadow-md shadow-primary/25">
                    {t("mostPopular")}
                  </span>
                </div>
              )}
                ...
                <div className="mb-4">
                  <div className="flex items-center gap-2.5 mb-1.5">
                    {'icon' in plan && plan.icon ? (
                      <div className={`flex h-9 w-9 items-center justify-center rounded-xl transition-transform duration-300 group-hover:scale-105 ${plan.popular ? 'bg-primary text-white shadow-md shadow-primary/20' : 'bg-purple-50 text-primary border border-purple-200/60'}`}>
              ...
              <div className="mb-4 p-3 rounded-xl bg-slate-50 border border-slate-200/60">
                ...
                <div className="flex items-baseline gap-1 mt-0.5">
                  ...
                  <span className="text-[10px] text-slate-400 font-medium">{plan.priceSuffix}</span>
                </div>
              </div>
              ...
              <ul className="mb-4 space-y-1.5">
                {plan.features.map((feature, i) => (
                  <li key={i} className="flex items-center gap-2">
                    <div className="flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-emerald-50 border border-emerald-200/60">
              ...
              <Link href="/register" onClick={handleButtonClick} className="block">
                <button
                  className={`w-full rounded-xl py-3 text-[12px] sm:text-[13px] font-semibold transition-all active:scale-[0.98] ${
                    plan.popular
                      ? "bg-primary text-white shadow-md shadow-primary/20 hover:bg-[#5800CC]"
