...
const ExchangeWidget = () => {
  ...
  return (
    <motion.div
      ...
      className="w-full max-w-[720px] bg-gradient-to-br from-white via-white to-slate-50/80 backdrop-blur-2xl rounded-2xl shadow-[0_24px_60px_-15px_rgba(0,0,0,0.12)] p-6 border border-slate-200/60 relative overflow-hidden"
    >
      <div className="absolute -top-24 -right-24 w-48 h-48 bg-gradient-to-br from-primary/15 to-[#6F00FF]/10 blur-[60px] rounded-full pointer-events-none" />
      <div className="absolute -bottom-16 -left-16 w-40 h-40 bg-gradient-to-tr from-emerald-400/10 to-primary/5 blur-[50px] rounded-full pointer-events-none" />
      
      <div className="flex items-center justify-between mb-5 relative z-10">
        <div className="flex items-center gap-3">
          <h3 className="text-lg font-bold text-slate-900 tracking-tight">Câmbio Instantâneo</h3>
        ...
        <span className="text-xs text-slate-400 font-medium">1 USDT = R$ {rate.toFixed(2)}</span>
      </div>

      <div className="flex items-stretch gap-3 relative z-10">
        <div className="flex-1 basis-0 min-w-0 bg-slate-50 rounded-xl p-4 border border-slate-200/80">
          <div className="flex items-center justify-between mb-2">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Você envia</span>
            <div className="flex items-center gap-1.5 bg-white px-2.5 py-1 rounded-full border border-slate-200">
              <img src="https://flagcdn.com/w20/br.png" alt="BR" className="w-4 h-3 rounded-sm object-cover" />
              <span className="text-xs font-bold text-slate-600">BRL</span>
            </div>
          </div>
        ...
        <div className="flex items-center flex-shrink-0">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-slate-900 to-slate-800 flex items-center justify-center shadow-lg">
            <ArrowRight className="w-5 h-5 text-white" />
          </div>
        </div>

        <div className="flex-1 basis-0 min-w-0 bg-gradient-to-br from-primary/[0.06] to-primary/[0.02] rounded-xl p-4 border border-primary/15">
          <div className="flex items-center justify-between mb-2">
            <span className="text-[10px] font-bold text-primary/50 uppercase tracking-wider">Você recebe</span>
            <div className="flex items-center gap-1.5 bg-white px-2.5 py-1 rounded-full border border-primary/15">
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.99 }}
        className="w-full mt-5 py-3.5 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 text-white text-sm font-bold rounded-xl shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-2 relative z-10 overflow-hidden group"
      >
